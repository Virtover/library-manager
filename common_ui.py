...
class BookTableUI:
    ...
    def refresh_table(self):
        ...
        for row in self.current_data:
            # Sanitize row data - convert None and NaN to empty strings, ensure correct number of columns
            sanitized_row = []
            for i in range(len(self.COLUMNS)):
                if i < len(row):
                    value = row[i]
                    # Convert None, NaN, and empty to empty string
                    ...
                    sanitized_row.append('')
            self.tree.insert('', 'end', values=tuple(sanitized_row))
    ...
    def apply_filter(self):
        ...
        # Build filter dict, removing empty values
        filter_dict = {
        ...
        self.current_filter = {k: v for k, v in filter_dict.items() if v}
